...
public class VariableJumpAbility : BaseAbility
{
    ...
    private void tryToJump(InputAction.CallbackContext value)
    {
        ...
        if (playerInput != null && playerInput.currentActionMap != null && playerInput.currentActionMap.name != "Player" || !isPermitted || linkedStateMachine.currentState == PlayerStates.State.Knockback || linkedStateMachine.currentState == PlayerStates.State.Reload)
            return;
